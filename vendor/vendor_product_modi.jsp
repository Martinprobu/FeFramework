						...
						<div class="row">
							<div class="col-xs-12">
								...
								<div class="table-header">
									...
									供应商 : <span style="font-weight:bold">${ requestPojo.vendorName }</span>  . 商品编辑
								</div>
																...
																<div class="col-sm-6">
																	<select multiple="" class="width-65 chosen-select tag-input-style" id="tagIdList" name="tagIdList" data-placeholder="点击选择标签...">
																			...
																			<c:forEach var="list" items="${tagList}" varStatus="status" >
																				<c:forEach var="mylist" items="${myTagList}">
																					<c:if test="${list.tagId == mylist.tagId }">
																					<c:set var="selectedId" value="${list.tagId}"></c:set>
																					</c:if>
																				</c:forEach>
																				<option value="${list.tagId}" name="tagIdListOpt" ${selectedId == list.tagId?'selected':'' }>${list.tag}</option>
																			</c:forEach>
...
<%-- 																	<input value="${requestPojo.recommend}" name="recommend" mustneed="true" id="recommend" type="hidden" />  --%>
															...
															<div class="form-group">
																...
																<div class="col-sm-6">
																	<input name="imageUrlOld" id="imageUrlOld" type="hidden" value="${requestPojo.imageUrl}">
													...
													<div id="catalogtab" class="tab-pane">
														已选类别：<c:forEach var="list" items="${myCatalogList}" varStatus="status" >
																${list.catalogName} , 
															</c:forEach>
															<p id = "catalogIds" style="display: none"><c:forEach var="list" items="${myCatalogList}" varStatus="status" >${list.catalogId},</c:forEach></p>
...
<!-- 																	<a href="#modal-form" role="button" class="blue" data-toggle="modal">查看属性模板</a> -->
														...
														<div id="paramAddBtnDiv">
															<c:forEach var="item" items="${myParameterList }">

																<div class="form-group">
																	<label class="col-sm-3 control-label no-padding-right"></label>
																	<div class="col-sm-3">
																		<input value="${item.paramName }" name="pro" type="text"
																			placeholder="请输入属性 名" class="col-xs-10 col-sm-4">
																		<input value="${item.paramValue }" name="provalue" type="text" placeholder="请输入属性 值"
																			class="col-xs-10 col-sm-4">
																		<input value="${item.paramUil }" name="util" type="text" placeholder="请输入单位"
																			class="col-xs-10 col-sm-4">
																	</div>
																</div>
															</c:forEach>
...
<!-- 																	<a href="#modal-form2" role="button" class="blue" data-toggle="modal" id="skuModule">查看SKU模板</a> -->
														...
														<div id="skuProModelDiv">
															<c:forEach var="list" items="${mySkuProList }">
																<div class="form-group">
																	<label class="col-sm-3 control-label no-padding-right" >
																		<input value="${list.proName }" name="skuPro" type="text" placeholder="请输入 组名" />
																	</label>
																	<div class="col-sm-8">
	<!-- 																	<a href="javascript:addSkuSimple();">+</a> 作用是在后面加一个框 一期先不做  -->
																		<c:forEach  var="item" items="${fn:split(list.proValueList,',') }">
																			<input value="${item }" name="skuProVal" type="text" placeholder="请输入 属性名" class="col-sm-3" />
																		</c:forEach>
																	</div>
																</div>
															</c:forEach>
													...
													<div id="descriptiontab" class="tab-pane">
														...
														<div style="display:none;" id="descHtmlInputDiv"></div>
														<input type="hidden" name="descHtmlInputOld" id="descHtmlInputOld" value="${requestPojo.descriptionUrl}">
                   ...
                   success: function (response) {
                	   var result = [];
	               	   	for(var i = 0; i<response.result.length; i++){
	               	   		var resultItem = response.result[i];
	               	   		var myCatalogStr = '${myCatalogStr}';
	               	   		var myCatalogList = myCatalogStr.split(',');
	               	   		for(var j=0;j<myCatalogList.length;j++){
	               	   			if(myCatalogList[j] == resultItem.id){
	               	   				resultItem.additionalParameters["item-selected"] = true;
	               	   			}
	               	   		}
...
// 			var reg = /^([0-9]\d*)(\.{0,1}\d*[1-9])?$/;   
	...
	var oldDetailMD5 = MD5($("#descHtml")[0].innerHTML);
	
	//更改标签(可以多个标签), 更改类别, 更改上传缩略图片,TOP图片, 更改商品基础信息, 更改SKU信息, 更改商品参数信息, 更改商品html详情信息
	$("#addSaveFunButton").click(function(){
		var newDetailMD5 = MD5($("#descHtml")[0].innerHTML);
		var newAllMD5 = MD5($("#productUpateForm").serialize());
		
		if (newAllMD5 === oldAllMD5 && newDetailMD5 == oldDetailMD5) {
...
//	       	console.log("cataHtmlList", cataHtmlList);
		...
		if(cataHtmlList == ""){
			var catalogIds = $("#catalogIds").html();
			$("#cataHtmlList").val(catalogIds.substring(0,catalogIds.length-1));
		}else{
...
//	       	return;
       	//Editor框
       	alert(newDetailMD5 +"----"+oldDetailMD5)
   		if(newDetailMD5 == oldDetailMD5){
   			$("#descHtmlInput").val("");
		}else{
